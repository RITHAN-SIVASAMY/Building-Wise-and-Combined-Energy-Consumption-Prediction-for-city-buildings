...
# Project Objective
print("🎯 Objective: Predict daily energy_sum across multiple buildings using Gradient Boosting with SHAP analysis.")
...
# --- Multiple Building Prediction with Gradient Boosting ---
print("\n=== Multiple Building Prediction (Gradient Boosting with SHAP) ===")
...
model = GradientBoostingRegressor(n_estimators=200, random_state=35)
...
for building_id in sample_building_ids:
    ...
    test_indices = X_test_ind.index  # Store test dates
    ...
    # --- Store Test Predictions for Last Building ---
    if building_id == sample_building_ids[-1]:
        test_dates = test_indices
        test_actual = y_test_ind
        test_predicted = y_pred_ind
...
if len(all_y_test) > 0 and len(all_y_pred) > 0 and len(all_dates) > 0:
    ...
    if len(filtered_plot_data) > 0:
        ...
        plt.title('Actual vs Predicted Energy Sum Across All Buildings (Gradient Boosting)')
        plt.legend()
        plt.xticks(rotation=45)
        plt.tight_layout()
        plot_path = os.path.join(output_dir, 'actual_vs_predicted_all_buildings_line_energy_sum_gb.png')
...
if len(all_y_test) > 0 and len(all_y_pred) > 0:
    ...
    if len(filtered_y_test) > 0:
        ...
        plt.title('Actual vs Predicted Energy Sum (0 to 0.5 kWh, Gradient Boosting)')
        plt.legend()
        plt.tight_layout()
        plot_path = os.path.join(output_dir, 'actual_vs_predicted_all_buildings_energy_sum_0to0_5_gb.png')
        plt.savefig(plot_path)
...
if len(all_y_test) > 0 and len(all_y_pred) > 0:
    if len(filtered_y_test) > 0:
        ...
        plt.xlabel('Actual Energy Sum (kWh)')
        plt.ylabel('Predicted Energy Sum (kWh)')
        plt.title('Regression Plot for Energy Sum (0 to 0.5 kWh, Gradient Boosting)')
        plt.legend()
        plt.tight_layout()
        plot_path = os.path.join(output_dir, 'regression_plot_all_buildings_energy_sum_0to0_5_gb.png')
        plt.savefig(plot_path)
...
if shap_values_list:
    ...
    plt.title("SHAP Summary Plot Across Buildings (Gradient Boosting)")
    plt.tight_layout()
    plot_path = os.path.join(output_dir, 'shap_summary_plot_energy_sum_gb.png')
    plt.savefig(plot_path)
    ...
    plt.tight_layout()
    plot_path = os.path.join(output_dir, 'shap_force_plot_last_building_energy_sum_gb.png')
    plt.savefig(plot_path)
...
plt.title('Distribution of R² Scores Across Buildings (Gradient Boosting)')
plt.tight_layout()
plot_path = os.path.join(output_dir, 'r2_distribution_energy_sum_gb.png')
plt.savefig(plot_path)
...
plt.xlabel('RMSE (kWh)')
plt.ylabel('Number of Buildings')
plt.title('Distribution of RMSE Across Buildings (Gradient Boosting)')
plt.tight_layout()
plot_path = os.path.join(output_dir, 'rmse_distribution_energy_sum_gb.png')
plt.savefig(plot_path)
...
if feature_importances:
    ...
    plt.ylabel('Feature')
    plt.title('Average Feature Importance Across Buildings (Gradient Boosting)')
    plt.tight_layout()
    plot_path = os.path.join(output_dir, 'feature_importance_energy_sum_gb.png')
    plt.savefig(plot_path)
    print(f"Saved Feature Importance plot: {plot_path}")
    plt.show()
    plt.close()

# Test Set Predictions vs Actual (Last Building)
if 'test_predicted' in locals():
    plt.figure(figsize=(10, 6))
    plt.plot(test_dates, test_actual, marker='o', color='blue', label='Actual Energy Sum')
    plt.plot(test_dates, test_predicted, marker='x', color='red', label='Predicted Energy Sum')
    plt.xlabel('Date')
    plt.ylabel('Energy Sum (kWh)')
    plt.title(f'Test Set Predictions vs Actual for Building {building_id} (Gradient Boosting)')
    plt.xticks(rotation=45)
    plt.legend()
    plt.tight_layout()
    plot_path = os.path.join(output_dir, 'test_predictions_last_building_energy_sum_gb.png')
    plt.savefig(plot_path)
    print(f"Saved Test Predictions plot: {plot_path}")
    plt.show()
    plt.close()

# 30-Day Future Prediction (Last Building)
if 'future_predictions' in locals():
    plt.figure(figsize=(10, 6))
    plt.plot(future_dates, future_predictions, marker='o', color='green', label='Predicted Energy Sum')
    plt.xlabel('Date')
    plt.ylabel('Energy Sum (kWh)')
    plt.title(f'30-Day Energy Sum Prediction for Building {building_id} (Gradient Boosting)')
    plt.xticks(rotation=45)
    plt.legend()
    plt.tight_layout()
    plot_path = os.path.join(output_dir, 'future_prediction_energy_sum_gb.png')
    plt.savefig(plot_path)
...
print("\n=== Aggregate R² and RMSE Analysis (Gradient Boosting, R² ≥ 0.7) ===")
